...
| `canvas.toDataURL` | records the call and returns a real minimal PNG (`pngFixture`) | Lets export tests check the exported size, and gives `savePNG()` genuine bytes to splice its `pHYs`/`sRGB` chunks into. `pngChunkTypes`/`pngChunkData` read them back out of the exported Blob. The fixture computes its own CRCs so a bug in the app's `crc32` cannot hide behind the same bug in the stub. |
...
| `savedFile(h)` | The file the app last handed to `<a download>`, whichever save built it — `{ name, href, type, revoked, bytes, text }`: the download name, the object URL, the Blob's MIME type, whether that URL has been revoked, and the bytes read back out of the real Blob. Async — `await` it. `text` is a lazy getter, so a chart's megabyte PNG is never decoded as UTF-8. |
| `savedScaleFile(h)` | The scale document the app last handed to `<a download>`, decoded out of its Blob — `{ name, text }`. Async — `await` it; jsdom's `Blob` has neither `text()` nor `arrayBuffer()`, so it reads through `FileReader`. |
| `savedAudioFile(h)` | The same as `savedFile(h)`, named for the **Save Audio As WAV** tests that read it. Async — `await` it. |
