...
| `savedAudioFile(h)` | The WAV file the app last handed to `<a download>` for **Save Audio As WAV** — `{ name, type, revoked, bytes }`: the download name, the Blob's MIME type, whether its object URL has been revoked, and the bytes themselves, read back through the `Blob`/`URL.createObjectURL` stub above. Async — `await` it. |
