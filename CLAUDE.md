...
- **Reload sync** (`syncDerivedStateToControls`): nothing is persisted — the DOM *is* the model — but a browser restores the controls across a reload, and it does so **after** `load`, firing no `change`. So everything derived from a control (editor row layout, cents labels, EDO row, swatch palette, zoom, notation class) is re-derived at load and again on `pageshow`. This is why `onIntervalTypeChange`/`onChartStyleChange` are each split from an `apply…` half. See `docs/ARCHITECTURE.md` § Startup and reload.
