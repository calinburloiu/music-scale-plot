...
- `byzantine.js` — Byzantine symbol model: the note/genus/fthora tables, `MARTYRIA_COMPATIBILITY`, the SBMuFL resolvers, and the note ladder. No DOM.
...
- **Notation** (`#notation`, `generic` or `byzantine`): switching notation never rebuilds the editor — every note row always carries both a name input and both symbol wells; CSS decides which half shows. See `docs/BYZANTINE-SYMBOLS.md` for the symbol model, the hand-maintained `MARTYRIA_COMPATIBILITY` table, the note ladder, and what adding a second font would touch.
