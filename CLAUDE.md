...
- `persistence-ui.js` — the DOM half of file persistence: the toolbar's handles, the Save menu, the message bar, `collectDocumentState`/`applyDocumentState`, New/Open/Save, the file keyboard shortcuts, `saveFileAs` — the one save every format in the app goes through — and `downloadBlob`, its `<a download>` half. Built on `persistence.js`.
...
- **Saving a file**: all three saves — `.musp.json`, PNG and WAV — hand their bytes to `saveFileAs()` (`persistence-ui.js`), passing only what is their own: the contents, the media type, the picker's file-type list and the wording of the failure. It proposes one name for all three — `suggestedFileName()` slugged from `#scale-name`, differing only in extension — and picks the route: where `showSaveFilePicker` exists it is the browser's own Save dialog; Safari, Firefox and every `file://` page reach `downloadBlob()`, which downloads a `Blob` behind an object URL, revoked on the next macrotask. **Never a `data:` URL**: iOS Safari's download manager will not fetch one, so the sheet appears and then nothing is written, silently. See `docs/ARCHITECTURE.md`'s "Handing the browser a file".
